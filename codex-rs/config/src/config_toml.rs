...
pub struct ConfigToml {
    ...
    /// Maximum number of output tokens generated by one Responses request.
    pub model_max_output_tokens: Option<u64>,

    /// Maximum number of output tokens generated by one *compaction* request.
    ///
    /// Compaction emits a summary, not working output, so it does not need the
    /// same output budget as a normal turn. On a slow local runtime this is the
    /// dominant cost of a compaction: at a measured 10-13 tok/s, letting a
    /// summary run to an 8000-token budget is 10+ minutes of generation. Falls
    /// back to `model_max_output_tokens` when unset.
    pub compact_model_max_output_tokens: Option<u64>,
    ...
    /// Optional reasoning-effort override used specifically for compaction
    /// requests (both local and remote). When unset, compaction falls back to
    /// `model_reasoning_effort`. Useful for local/self-hosted models where
    /// compaction is a summarization task that does not need the same
    /// reasoning depth as normal coding turns, so it can run with a cheaper
    /// (e.g. `low`) effort without changing normal-turn behavior.
